...
class Storage(object):
    ...
    def _run_migrations(self, current_migration_version: int):
        ...
        while current_migration_version < latest_migration_version:
            ...
            migration = importlib.import_module(
                f".migrations.{str(next_migration_version).rjust(3, '0')}", "middleman"
            )
            # noinspection PyUnresolvedReferences
            ...
            self._execute(
                "UPDATE migration_version SET version = ?", (next_migration_version,)
            )
    ...
    def get_message_by_management_event_id(
        self, management_event_id: str
    ) -> Optional[dict]:
        self._execute(
            "SELECT room_id, event_id FROM messages where management_event_id = ?",
            (management_event_id,),
        )
        row = self.cursor.fetchone()
        if row:
